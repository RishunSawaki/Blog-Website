...
export default async function TagPage({ params }: Readonly<{ params: Promise<{ locale: string; tag: string }> }>) {
  ...
  const posts = (await getAllPosts(locale as Locale)).filter((post) => post.tags.includes(tag));

  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      <p className="text-sm uppercase tracking-[0.25em] text-foreground/50">{messages.tagsTitle}</p>
      <h1 className="mt-3 font-display text-4xl font-semibold text-foreground">#{tag}</h1>
